...
export P_CI_DIR="${BASE_BUILD_DIR}/bitrupee-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/bitrupee-config.h > src/config/bitrupee-config.h.tmp && mv src/config/bitrupee-config.h.tmp src/config/bitrupee-config.h'
fi
