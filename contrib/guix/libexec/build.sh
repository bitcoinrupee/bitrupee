...
make -C depends --jobs="$JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform bitrupee-linux-g++' \
                                   qt_config_opts_x86_64_linux='-platform linux-g++ -xplatform bitrupee-linux-g++' \
...
(
    ...
    # Setup the directory where our Bitrupee Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Bitrupee Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
