...
This guide describes how to build bitrupeed, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Bitrupee Core from source.
...
### 4. Clone Bitrupee repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Bitrupee Core repository to a directory.
...
It is not necessary to build wallet functionality to run `bitrupeed` or  `bitrupee-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/bitrupee/bitrupee/blob/master/doc/descriptors.md), `sqlite` is required.
...
Bitrupee Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/bitrupee/bitrupee/issues/7714).
...
You can deploy a `.dmg` containing the Bitrupee Core application using `make deploy`.
...
## Building Bitrupee Core

### 1. Configuration

There are many ways to configure Bitrupee Core, here are a few common examples:
...
Run the following in your terminal to compile Bitrupee Core:
...
## Running Bitrupee Core

Bitrupee Core should now be available at `./src/bitrupeed`.
If you compiled support for the GUI, it should be available at `./src/qt/bitrupee-qt`.

The first time you run `bitrupeed` or `bitrupee-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Bitrupee/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Bitrupee"

touch "/Users/${USER}/Library/Application Support/Bitrupee/bitrupee.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Bitrupee/bitrupee.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Bitrupee/debug.log
```

## Other commands:

```shell
./src/bitrupeed -daemon      # Starts the bitrupee daemon.
./src/bitrupee-cli --help    # Outputs a list of command-line options.
./src/bitrupee-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/bitrupee-qt -server # Starts the bitrupee-qt server mode, allows bitrupee-cli control
```
