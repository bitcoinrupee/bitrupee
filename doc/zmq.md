...
The Bitrupee Core daemon can be configured to act as a trusted "border
router", implementing the bitrupee wire protocol and relay, making
...
The ZeroMQ feature in Bitrupee Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitrupeed:
...
For instance:
    ...
    $ bitrupeed -zmqpubhashtx=tcp://127.0.0.1:226145 \
               -zmqpubhashtx=tcp://192.168.1.2:226145 \
               -zmqpubrawtx=ipc:///tmp/bitrupeed.tx.raw \
...
These options can also be provided in bitrupee.conf.
...
From the perspective of bitrupeed, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitrupeed directly. Furthermore, no information is
...
using. Bitrupeed appends an up-counting sequence number to each
