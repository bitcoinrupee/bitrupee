# Fuzzing Bitrupee Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Bitrupee Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/bitrupee/bitrupee
$ cd bitrupee/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/bitrupee/bitrupee/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/bitrupee/bitrupee/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/bitrupee/bitrupee/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/bitrupee/bitrupee/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`bitrupee-core/qa-assets`](https://github.com/bitrupee-core/qa-assets) repo.

To fuzz `process_message` using the [`bitrupee-core/qa-assets`](https://github.com/bitrupee-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/bitrupee-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`bitrupee-core/qa-assets`](https://github.com/bitrupee-core/qa-assets) repo.

Every single pull request submitted against the Bitrupee Core repo is automatically tested against all inputs in the [`bitrupee-core/qa-assets`](https://github.com/bitrupee-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Bitrupee Core more robust.
...
with certain assembly code from Bitrupee Core's code. See [developer notes on sanitizers](https://github.com/bitrupee/bitrupee/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Bitrupee Core using afl++

## Quickstart guide

To quickly get started fuzzing Bitrupee Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Bitrupee Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Bitrupee Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Bitrupee Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Bitrupee
Core without having to write any custom fuzzing harness. The `bitrupeed` server
...
```sh
$ mkdir bitrupee-honggfuzz-p2p/
$ cd bitrupee-honggfuzz-p2p/
$ git clone https://github.com/bitrupee/bitrupee
$ cd bitrupee/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/bitrupeed.cpp b/src/bitrupeed.cpp
...
Bitrupee Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitrupee-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=bitrupee-core).
...
because Bitrupee is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_bitrupee-core/latest).
