...
Bitrupee Core version 0.20.0 is now available from:

  <https://bitrupeerupeecore.org/bin/bitrupeerupee-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Bitrupee Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bitrupee Core version 0.18.0.
...
- Testing or debugging of implementations of the Bitrupee P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bitrupee Core. Bitrupee Core logs debug messages
...
- OpenSSL is no longer used by Bitrupee Core.  (#17265)

- BIP70 support has been fully removed from Bitrupee Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitrupeerupee/bitrupeerupee/issues/16599),
  [PR #16702](https://github.com/bitrupeerupee/bitrupeerupee/pull/16702), and the
  `bitrupeerupeed help` for more information.  This option is experimental and
...
- All custom settings configured when Bitrupee Core starts are now
...
- The "Start Bitrupee Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitrupeerupeecore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Bitrupee Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bitrupee Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitrupeerupee-core/HWI) for
  signing.  Future versions of Bitrupee Core should support a GUI option
...
- Bitrupee Core's automatically-generated source code documentation is
  now available at https://doxygen.bitrupeerupeecore.org.  (#17596)
...
- The `bitrupeerupee-cli` utility used with the `-getinfo` parameter now
...
- The `bitrupeerupee-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitrupeerupeestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitrupeerupee-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_bitrupeerupee` binary (hebasto)
...
- #18702 Fix ASLR for bitrupeerupee-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitrupeerupee schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitrupeerupee.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitrupeerupee` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitrupeerupee-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitrupeerupee-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bitrupee Core files (hebasto)
...
- #17119 Fix broken bitrupeerupee-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitrupeerupee-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitrupeerupeecore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitrupeerupee-wallet (hel-o)
...
- #16115 On bitrupeerupeed startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitrupeerupee-cli -getinfo (malevolent)
