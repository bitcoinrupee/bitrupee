Bitrupee Core version 0.19.0.1 is now available from:

  <https://bitrupeerupeecore.org/bin/bitrupeerupee-core-0.19.0.1/>
...
to open the Bitrupee Core .dmg. This is due to new signing requirements
imposed by Apple, which the Bitrupee Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitrupeerupee/bitrupeerupee/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Bitrupee Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Bitrupee
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitrupeerupeed -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Bitrupee Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Bitrupee Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Bitrupee Core. (#14054)

- To eliminate well-known denial-of-service vectors in Bitrupee Core,
  ...
  This prevents Bitrupee Core from sending the BIP111 NODE_BLOOM service
  ...
  of Bitrupee Core and nodes that have manually enabled BIP37 support,
...
- By default, Bitrupee Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `bitrupeerupee-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Bitrupee Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Bitrupee Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Bitrupee Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Bitrupee address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Bitrupee Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Bitrupee AEAD (jonasschnelli)
...
- #15639 bitrupeerupee-wallet tool: Drop libbitrupeerupee_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in bitrupeerupeed/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitrupeerupee-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bitrupeerupeed build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bitrupeerupee_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitrupeerupee to auto generated project files (sipsorcery)
...
- #15773 Add BitrupeeTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitrupeerupee output (practicalswift)
...
- #15896 feature_filelock, interface_bitrupeerupee_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitrupee Core (luke-jr)
...
- #16277 Suppress output in test_bitrupeerupee for expected errors (gertjaap)
...
- #16489 log: harmonize bitrupeerupeed logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitrupeerupeeVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitrupeerupee.conf (fanquake)
...
- #16080 build/doc: update bitrupeerupee_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitrupeerupee.conf (fanquake)
...
- #16621 add default bitrupeerupee.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitrupeerupee-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitrupeerupee.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bitrupeerupeed.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitrupeerupee/bitrupeerupee/).
