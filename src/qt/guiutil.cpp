...
#include <qt/bitrupeeaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitrupee address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BitrupeeAddressEntryValidator(parent));
    widget->setCheckValidator(new BitrupeeAddressCheckValidator(parent));
}
...
bool parseBitrupeeURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitrupee: URI
    if(!uri.isValid() || uri.scheme() != QString("bitrupee"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitrupeeUnits::parse(BitrupeeUnits::BTR, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitrupeeURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitrupeeURI(uriInstance, out);
}

QString formatBitrupeeURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bitrupee:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitrupeeUnits::format(BitrupeeUnits::BTR, info.amount, false, BitrupeeUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitrupeeConf()
{
    ...
    /* Open bitrupee.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitrupee.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitrupee (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Bitrupee (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Bitrupee*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitrupee.desktop";
    return GetAutostartDir() / strprintf("bitrupee-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bitrupee.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Bitrupee\n";
        else
            optionFile << strprintf("Name=Bitrupee (%s)\n", chain);
