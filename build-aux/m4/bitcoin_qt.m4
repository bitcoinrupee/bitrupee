dnl Copyright (c) 2013-2016 The Bitrupee Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set bitrupee_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitrupee_qt_want_version" = xauto && test "x$bitrupee_qt_force" != xyes; then
    if test "x$bitrupee_enable_qt" != xno; then
      AC_MSG_WARN([$1; bitrupee-qt frontend will not be built])
    fi
    bitrupee_enable_qt=no
    bitrupee_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$bitrupee_enable_qt" != xno && test "x$bitrupee_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build bitrupee-qt GUI (default=auto)])],
    [
     bitrupee_qt_want_version=$withval
     if test "x$bitrupee_qt_want_version" = xyes; then
       bitrupee_qt_force=yes
       bitrupee_qt_want_version=auto
     fi
    ],
    [bitrupee_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); bitrupee_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: bitrupee_enable_qt, bitrupee_enable_qt_dbus, bitrupee_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$bitrupee_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    bitrupee_enable_qt=yes
    bitrupee_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      bitrupee_enable_qt_test=no
    fi
    bitrupee_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      bitrupee_enable_qt_dbus=yes
    fi
  ...
  if test x$bitrupee_enable_qt = xyes; then
    AC_MSG_RESULT([$bitrupee_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$bitrupee_enable_qt])
  fi
...
dnl Output: bitrupee_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, bitrupee_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [bitrupee_cv_static_qt=yes],
      [bitrupee_cv_static_qt=no])
    ])
])
